generate_params_df <- function(preproc_data){
  section_size <- 4 * samp_rate #Calculate number of data points in one 8s window
  ...
  df <- data.frame(tci = rep(NA, no_sections), mav = rep(NA, no_sections), count1 = rep(NA, no_sections), count2 = rep(NA, no_sections),
                   count3 = rep(NA, no_sections), exp = rep(NA,no_sections), vfleak = rep(NA, no_sections),
                   cm = rep(NA, no_sections), cvbin = rep(NA, no_sections), frqbin = rep(NA, no_sections), 
                   abin = rep(NA, no_sections), kurt = rep(NA, no_sections), label = rep("",no_sections), stringsAsFactors = FALSE)
